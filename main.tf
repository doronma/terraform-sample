...
resource "aws_vpc" "vpc" {
  ...
  tags = {
    ...
    Terraform   = "true"
    Reigion     = data.aws_region.current.name
...
# Define another local module - server
...
# Create autoscalling group from module registry
module "autoscaling" {
...
# Create S3 Bucket from registry
module "s3-bucket" {
